...
const reverseStringsForObject = (object, meta) => {
  let template = meta.content.templates.find(tpl => tpl.code === object.template);
  if (!template) return;
  let fields = Object.values(template.fields).filter(field => field.type instanceof SString && field.type.useMacros && object.fields[field.code])
...
export const migrator = (content) => {
  let meta = {
    ...
    .filter((object) => object.template === 'stringReplaceRules' && object.fields.source && object.fields.result)
    .map((object) => {
      return {
